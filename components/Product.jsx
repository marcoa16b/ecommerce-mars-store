...
const Product = (product) => {
  ...
  const [isOnCartList, setIsOnCartList] = useState(false);
  ...
  const onCartlist = () => {
    ...
    cart.map(prod => {
      if (product.id === prod.id){
        found = true;
        setIsOnCartList(true);
      }
    })
    if (!found) setIsOnCartList(false);
  };
  ...
  return (
    <article className="border border-gray-200 rounded bg-white flex flex-col relative z-10">
      <button
